...
public class ShoppingFrame extends BaseFrame {
	...
	JButton btnBuy = createComponent(createButton("구매", this::clickBuy), 100, 30);
	JButton btnDelete = createComponent(createButton("삭제", this::clickDelete), 100, 30);

	public ShoppingFrame() {
		super(700, 350, "장바구니");

		add(createComponent(
				createLabel(new JLabel(userName + "님 장바구니", JLabel.CENTER), new Font("맑은고딕", Font.BOLD, 24)), 0, 45),
				BorderLayout.NORTH);
	...
	public void refresh() {
		...
		try (var pst = connection.prepareStatement(
			...
			while (rs.next()) {
				model.addRow(new Object[] { rs.getInt(1), rs.getString(7), rs.getInt(3), rs.getInt(4), rs.getString(5),
						rs.getInt(6) });
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void clickBuy(ActionEvent event) {
		...
		if (answer == JOptionPane.NO_OPTION) {
			try {
				statement.execute("INSERT INTO orderlist (" + "SELECT 0, CURDATE(), " + userNo
						+ ", shopping.m_no, m_group, s_size, s_price, s_count, s_amount FROM shopping "
			...
			userPoint -= amount;
			informationMessage("포인트로 결제되었습니다.\n남은 포인트 : " + userPoint);
		}
		...
		try {
			statement.execute("delete from shopping;");
	...
	public int getAmount() {
		...
		for (int i = 0; i < model.getRowCount(); i++) {
			amount += (int) model.getValueAt(i, 5);
		}
		return amount;
	}
